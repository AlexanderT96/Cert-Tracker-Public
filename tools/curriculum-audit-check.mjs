...
assert.equal(route.ids.length,27,'Focused route must contain 25 prior milestones plus CWNA and Meraki');
...
assert.equal(summary.coreScope,27);assert.equal(summary.coreVerified,22);assert.equal(summary.coreRestricted,5);assert.equal(summary.focusScope,5);assert.equal(summary.focusVerified,5);
...
assert.equal(route.focusTracks.find(x=>x.id==='cwnp-wifi').certs.join(','),'cwap,cwdp,cwsp,cwisa,cwne','CWNP depth must remain outside core My Path with the CWNE prerequisite gate visible');
...
for(const id of route.focusTracks.flatMap(track=>track.certs))if(!['cwna','cisco-meraki-solutions'].includes(id))assert.equal(route.ids.includes(id),false,`${id}: optional focus credential must not bloat core My Path`);
