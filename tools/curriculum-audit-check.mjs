...
assert.equal(route.ids.length,32,'Focused route must contain the complete visible CWNP progression plus Cisco wireless milestones');
...
assert.equal(summary.coreScope,32);assert.equal(summary.coreVerified,27);assert.equal(summary.coreRestricted,5);assert.equal(summary.focusScope,0);assert.equal(summary.focusVerified,0);
