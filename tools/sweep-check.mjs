...
const pathwayResourceGaps=[];
for(const id of pathwayProfiles){const profile=CT.learningResources.profile(certs.find(cert=>cert.id===id));if(!profile.subjects.every(subject=>subject.resourceIntegrity.substantive>0))pathwayResourceGaps.push(id);}
console.log(JSON.stringify({certifications:certs.length,roles:CT.careerOptions.ROLES.length,unavailable:Object.fromEntries(unavailable),routeUnavailable,defaultUnavailable,pathwayDefinitions:pathwayDefinitionIds.length,pathwayUnknown,pathwayResourceGaps,subjects,resources,references,discovery:discoveryCount,externalCandidates,reviewed,platforms,tutorCheckpoints,subjectsWithoutStudyBrief,subjectsWithoutTutorDecision,nonSubjects},null,2));
...
assert.equal(pathwayResourceGaps.length,0,`Every pathway credential needs substantive subject resources: ${pathwayResourceGaps.join(', ')}`);
