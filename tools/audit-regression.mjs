...
assert.deepEqual(Array.from(CT.focusedRoute.definition.focusTracks.find(x=>x.id==='ot-convergence')?.certs||[]),['iec-62443-cfs','iec-62443-cra','iec-62443-cds','iec-62443-cms','iec-62443-expert','isa95-fund','isa-apm','isa-cap-associate','isa-cap','isa-61511-sis-fund','isa-61511-sil-select','isa-61511-sil-verify','isa-61511-expert']);
...
assert.equal(CT.focusedRoute.definition.focusTracks.find(x=>x.id==='milestone-briefcam'),undefined);assert.ok(expected.includes('briefcam-tech'));
...
assert.equal(cert('ai-901').code,'AI-901');assert.deepEqual(Array.from(cert('ai-901').deps),['pcep']);assert.deepEqual(Array.from(cert('ai-103').deps),['pcap','ai-901']);assert.equal(cert('pcep').validity,60);assert.equal(cert('pcap').validity,60);
...
assert.ok(learning.includes('Employer-access unlock tracks · 1')&&learning.includes('LenelS2 access-control engineering ecosystem')&&!learning.includes('Milestone BriefCam video-analytics ecosystem'),'Only currently inaccessible physical-security ecosystems belong in the collapsed unlock group');
assert.ok(learning.includes('LCA')&&learning.includes('Milestone BriefCam Technical Certification'),'Restricted LenelS2 and core BriefCam certification records must render');
...
assert.equal(previous.length,25);assert.equal(expected.length,35);
...
CT.focusedRoute.apply();state.passes=Object.fromEntries(expected.slice(0,15).map(id=>[id,'2026-01-01']));
