...
try{
  ...
  assert.equal(await focused.page.evaluate(()=>Object.keys(state.myPath).length),32);
  ...
  assert.equal(await focused.page.locator('[data-focused-route] li').count(),32);
  await focused.page.evaluate(()=>{
  ...
  await focused.page.reload();await navigationInViewport(focused.page);
  assert.equal(await focused.page.evaluate(()=>Object.keys(state.myPath).length),32,'Previous focused route upgrades without removing milestones');
  assert.equal(await focused.page.evaluate(()=>state.notes.ccna.text),'Preserve route adoption note');
