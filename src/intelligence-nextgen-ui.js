...
(function initNextGenIntelligenceUI(global){
  ...
  function timelineLabel(s){const last=s.last?.subject||'No current item';return `${s.count} event${s.count===1?'':'s'} · ${s.active} active · ${last}`;}
  function feedbackControls(key){if(!key)return '';return `<div class="ct-ng-feedback"><button type="button" data-ng-feedback="VALUABLE" data-ng-key="${esc(key)}">Useful</button><button type="button" data-ng-feedback="SNOOZED" data-ng-key="${esc(key)}">Snooze weight</button><button type="button" data-ng-feedback="DISMISSED" data-ng-key="${esc(key)}">Dismiss</button></div>`;}
  function html(snap){
    const cal=snap.calibration,attention=snap.attention,story=snap.storylines.slice(0,5),packs=snap.evidencePacks.slice(0,4),pattern=snap.patterns.slice(0,4),horizonKeys=['TODAY','7D','30D','90D','12M','LONG'];
    return `<section class="ct-ng" data-nextgen-intelligence aria-label="Next-generation intelligence">
      ...
      <div class="ct-ng-grid">
        ...
        <div class="ct-ng-card"><h3>Evidence-pack candidates</h3>${packs.length?packs.map(x=>`<div class="ct-ng-row"><div><strong>${esc(x.subject)}</strong><small>${esc(x.pillar)} · ${esc(x.horizon)} · confidence ${x.confidence}% · source ${x.sourceReliability}%${x.nextAction?` · ${esc(x.nextAction)}`:''}</small>${feedbackControls(x.feedbackKey)}</div><span class="ct-ng-score">${x.score}</span></div>`).join(''):'<div class="ct-ng-note">No high-value decisions currently need a compact evidence pack.</div>'}</div>
      </div>
      <div class="ct-ng-grid">
  ...
  function render(){
    ...
    const section=wrap.firstElementChild;bind(section);
    const pi=content.querySelector('[data-personal-intelligence]');if(pi)pi.after(section);else content.prepend(section);
  }
