...
(function initLearningResourcesUI(global){
  ...
  function auditBlock(p){const a=p.audit;if(!a)return'';const status=String(a.status||'QUEUED').replaceAll('_',' '),verified=a.status==='VERIFIED',restricted=a.status==='ACCESS_RESTRICTED',label=verified?'BLUEPRINT CROSS-WALK':restricted?'PUBLIC COVERAGE FLOOR':'COVERAGE REVIEW';const domains=(verified||restricted)?a.domains.map(row=>`<li><strong>${esc(row.subject)}</strong><span>${esc(row.objective)}${row.weight?` · ${row.weight[0]}${row.weight[1]!==row.weight[0]?`–${row.weight[1]}`:''}%`:''}</span></li>`).join(''):'';return `<section class="ct-blueprint-audit ${verified?'verified':'pending'}"><div class="ct-blueprint-audit-head"><div><span>${esc(label)}</span><strong>${esc(status)}${a.version?` · ${esc(a.version)}`:''}</strong></div><a href="${esc(a.url)}" target="_blank" rel="noopener noreferrer">Official source ↗</a></div><p>${esc(a.note)}${a.checkedAt?` Checked ${esc(a.checkedAt)}.`:''}${restricted?' Exact exam weighting remains locked until the issuer curriculum is inspected.':''}</p>${domains?`<ol>${domains}</ol>`:''}</section>`;}
